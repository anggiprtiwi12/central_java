...
const ReturnProduk = () => {
  ...
  const filteredData = riwayatData.filter((data) => data.status === "Return");

  const cardData = [
    {
      title: "Total Produk Dikembalikan",
      value: filteredData.length,
      date: new Date().toLocaleDateString(),
    },
    {
      title: "Total Pendapatan Dikembalikan",
      value: filteredData.reduce((acc, data) => acc + data.jumlah, 0).toLocaleString(),
      date: new Date().toLocaleDateString(),
    },
    {
      title: "Total Jumlah Produk Dikembalikan",
        ...
        <div className="container mx-auto px-4 flex">
          ...
          <div className="flex-1">
            <div className="mb-4 flex justify-between items-center">
              <h2 className="text-2xl font-bold">Return Produk</h2>
                ...
                <tbody>
                  {filteredData.map((data, index) => (
                    <tr key={index}>
                      ...
                      <td className="border-b py-2 px-4 text-orange-500">{data.status}</td>
